...
<div class="r_comments">
	<ul>
		<?php
			...
			$output = $pre_HTML;
			foreach ($comments as $comment) {$output .= "\n<li><b>".get_avatar( $comment, 32 )." <a href=\"" . get_permalink($comment->ID) ."#comment-" . $comment->comment_ID . "\" title=\"ئورنى: " .$comment->post_title . "\">" .strip_tags($comment->comment_author)."：</b><br />". strip_tags($comment->com_excerpt)."</a></li>";}
			$output .= $post_HTML;
			echo $output;
		?>
	</ul>
